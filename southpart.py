...
def get_page_info(file):
    ...
    with st.spinner(f'Getting the list of locations...'):
        ...
        for page in range(num_pages):
            st.spinner(f'Getting the list of locations...{page}')
            #st.markdown(page)
            ...
            loc_count.text(f'Total Locations: {n_locs}')
            ...
            if (page+1)<num_pages:
                c = int((100*(page+1)/num_pages)+1)
                #s = page_text.split()[page]
                s = '...' if page%5==0 else ('.'*5 if page%9==0 else '..')
                count.write(f'{c}% '+s)
                #bar.progress(int((100*(page+1)/num_pages)+1))
            else:
                count.text('')
